...
def load_condition_C_prime(data_dir: str = "data") -> List[str]:
    folder = Path(data_dir) / "condition_C_prime"
    texts = []
    for p in sorted(folder.glob("*.txt")):
        texts.append(p.read_text(encoding="utf-8").strip())
    logger.info(f"Condition C_prime loaded: {len(texts)} control prompts")
    return texts
...
def load_all(data_dir: str = "data") -> Dict:
    return {
        ...
        "C_prime": load_condition_C_prime(data_dir),
    }
...
def verify_data(data: Dict) -> bool:
    ...
    if len(data["C_prime"]) != 3:
        logger.error(
            f"Condition C_prime has {len(data['C_prime'])} controls, expected exactly 3"
        )
        ok = False

    # Token length rough check (1 word ≈ 1.3 tokens)
    len_A = len(data["A"][0].split())
    for i, text in enumerate(data["C_prime"]):
        len_C = len(text.split())
        ratio = len_C / len_A
        if ratio < 0.7 or ratio > 1.3:
            logger.warning(
                f"Condition C_prime_{i+1} length ratio vs A: {ratio:.2f} "
                f"(target 0.85–1.15)"
            )
