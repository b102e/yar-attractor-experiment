...
def save_results(distance_results: dict, data: dict) -> str:
...
def print_summary(distance_results: dict):
    ...
    print("=" * 60)
    print(f"{'Layer':<8} {'Within A+B':>12} {'Between':>12} {'p-value':>10} {'d':>8} {'Sig':>6}")
    print("-" * 60)

    for layer in sorted(distance_results.keys()):
        s = distance_results[layer]["stats"]
        sig = "✓" if s["significant"] else "✗"
        print(
            f"{layer:<8} "
            f"{s['mean_within_AB']:>12.4f} "
            f"{s['mean_between']:>12.4f} "
            f"{s['p_value']:>10.4f} "
            f"{s['cohens_d']:>8.3f} "
            f"{sig:>6}"
        )
    ...
    within_means = [distance_results[l]["stats"]["mean_within_AB"]
    ...
    # H3: distilled core
    print("\nExploratory H3 — Distilled core distance to A+B centroid:")
    for layer in sorted(distance_results.keys()):
        d = distance_results[layer]["d_to_centroid_AB"]
        w = distance_results[layer]["stats"]["mean_within_AB"]
        flag = "≈ within" if d < w * 1.5 else "far"
        print(f"  Layer {layer}: {d:.4f}  ({flag})")
...
def main():
    ...
    if args.skip_extraction:
        ...
        activations = extract_all(
            model, tokenizer, data, CONFIG.layers, CONFIG.activations_dir
        )
        del model  # free VRAM
    ...
    # 4. Save results
    save_results(distance_results, data)
