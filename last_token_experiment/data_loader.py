...
Expected structure:
    data/
        ...
        condition_C_hybrid/
            C1_hybrid.txt ... C7_hybrid.txt  # C controls with Yar command schema
        condition_D.txt          # distilled core
        condition_D_random.txt   # random 5-sentence slice from A
"""
...
def load_condition_C_hybrid(data_dir: str = "data") -> List[str]:
    folder = Path(data_dir) / "condition_C_hybrid"
    texts = []
    for p in sorted(folder.glob("*.txt")):
        texts.append(p.read_text(encoding="utf-8").strip())
    logger.info(f"Condition C_hybrid loaded: {len(texts)} control prompts")
    return texts


def load_condition_D_random(data_dir: str = "data") -> str:
    path = Path(data_dir) / "condition_D_random.txt"
    text = path.read_text(encoding="utf-8").strip()
    logger.info(f"Condition D_random loaded: {len(text.split())} words")
    return text
...
def load_all(data_dir: str = "data") -> Dict:
    return {
        ...
        "C_hybrid": load_condition_C_hybrid(data_dir),
        "D": [load_condition_D(data_dir)],
        "D_random": [load_condition_D_random(data_dir)],
    }
...
def verify_data(data: Dict) -> bool:
    ...
    if len(data["C_hybrid"]) < 5:
        logger.error(
            f"Condition C_hybrid has only {len(data['C_hybrid'])} controls, need at least 5"
        )
        ok = False
    ...
    if len(data["D_random"]) != 1:
        logger.error("Condition D_random must have exactly 1 document")
        ok = False
    ...
    for i, text in enumerate(data["C_hybrid"]):
        len_C = len(text.split())
        ratio = len_C / len_A
        if ratio < 0.7 or ratio > 1.3:
            logger.warning(
                f"Condition C_hybrid{i+1} length ratio vs A: {ratio:.2f} "
                f"(target 0.85–1.15)"
            )
